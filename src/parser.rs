...
pub fn parse_plain_text(file: &str) -> ParsedFile {
    ...
    let mut previous_longest_category = 0;
    let mut longest_category = 0;
    ...
    for (i, line) in lines.enumerate() {
        ...
        match parse_line(line) {
            Some(bookmark) => {
                ...
                if title_char_count > longest_title {
                    previous_longest_title = longest_title;
                    longest_title = title_char_count;
                }
                let category_char_count = bookmark.category().chars().count();
                if category_char_count > longest_category {
                    previous_longest_category = longest_category;
                    longest_category = category_char_count;
                }
    ...
    ParsedFile {
        bookmarks,
        invalid_lines,
        previous_longest_title,
        longest_title,
        previous_longest_category,
        longest_category,
    }
}
...
mod tests {
    ...
    fn test_formatted_line() {
        ...
        let formatted_line = bookmark.formatted_line(25, 25);
    ...
    fn test_parse_line() {
        let default_bookmark = Bookmark::default();
        let bookmark = parse_line(&default_bookmark.formatted_line(10, 10)).unwrap();
